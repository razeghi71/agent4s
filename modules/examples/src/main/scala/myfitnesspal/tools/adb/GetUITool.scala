...
case class GetUIInput(
    ...
    @description(
      "If true, returns raw XML instead of compact representation. Default is false (compact)."
    )
...
case class GetUIOutput(
    success: Boolean,
    @description(
      "Compact UI representation with actionable elements only (or raw XML if rawXml=true)"
    )
...
/** Dump and retrieve UI hierarchy from Android device
  ...
  *
  * Compact format: [id] label (class) [actions] @ [bounds] Example: [2] Log
  * Food (View) [click] @ [228,1311][333,1416]
  */
class GetUITool[F[_]: Async] extends Tool[F, GetUIInput, GetUIOutput]:
  ...
  private def pullUIDump(deviceId: Option[String]): F[String] =
    Async[F].blocking {
      ...
      try
        val pullCmd =
          s"adb ${deviceFlag}pull /sdcard/window_dump.xml ${tempFile.getAbsolutePath}"
        Process(pullCmd).!!
