...
import no.marz.agent4s.llm.provider.openai.OpenAIResponsesProvider
import com.melvinlow.json.schema.annotation.description
import com.melvinlow.json.schema.generic.auto.given
import io.circe.generic.auto.given

/** Simple example demonstrating the OpenAI Responses API with GPT-5.2 */
  ...
  private def evaluateExpression(expr: String): Double =
    ...
    try
      ...
      if engine != null then
        ...
        // Fallback: just parse as double if it's a simple number
        cleaned.trim.toDouble
    catch
      case _: Exception => 0.0

// Define a greeting tool  
case class GreetingInput(
...
case class GreetingOutput(greeting: String)

object GreetingTool extends Tool[IO, GreetingInput, GreetingOutput]:
  def name: String = "greet"
  def description: String = "Generates a personalized greeting in the specified language"

  def execute(input: GreetingInput): IO[GreetingOutput] =
    IO {
...
@main def responsesApiExample(): Unit =
  println("\n=== OpenAI Responses API Example (GPT-5.2) ===\n")

  val program = OpenAIResponsesProvider.resourceFromEnv[IO].use { provider =>
    // Setup tool registry
    given toolRegistry: ToolRegistry[IO] = ToolRegistry
      .empty[IO]
      .register(CalculatorTool)
      .register(GreetingTool)

    // Example 1: Simple chat without tools
    println("--- Example 1: Simple Chat ---")
    val simpleRequest = ChatCompletionRequest(
      model = "gpt-4o", // Use gpt-5.2 when available
      messages = Seq(
