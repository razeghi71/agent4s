...
@main def hello(): Unit =
  val result = OpenAIProvider.resourceFromEnv[IO].use { llmProvider =>
    // Setup tool registry
    given toolRegistry: ToolRegistry[IO] =
      ToolRegistry.empty[IO].register(GetWeatherTool)

    // Create nodes
    val chatNode = new ChatNode(llmProvider, toolRegistry)
