...
class ToolRegistrySuite extends munit.FunSuite:
  ...
  test("Reject duplicate tool names") {
    ...
    interceptMessage[IllegalArgumentException]("Tool 'add' is already registered") {
  ...
  test("Tool not found error (returnErrorsAsToolMessages = true)") {
    ...
    val result = registry.execute(toolCall, returnErrorsAsToolMessages = true).unsafeRunSync()
    ...
    assertEquals(content.hcursor.get[String]("type").toOption, Some("RuntimeException"))
  }
  ...
  test("Tool not found error (returnErrorsAsToolMessages = false)") {
    ...
    interceptMessage[RuntimeException]("Tool not found: 'nonexistent'") {
      registry.execute(toolCall, returnErrorsAsToolMessages = false).unsafeRunSync()
    }
  }
  ...
  test("JSON decode failure (returnErrorsAsToolMessages = false)") {
    ...
    assert(thrown.getMessage.contains("Failed to decode tool arguments for 'add'"))
  }
